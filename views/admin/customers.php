...
if(isset($_GET['id'])) {
    ...
    $statement = $pdo->prepare("SELECT * FROM personnes where role='client'");
    $statement->execute();
    if($statement->rowCount() > 0) {
        $customers = $statement->fetchAll(PDO::FETCH_ASSOC);
    }
...
<div class="container">
    <div class="page-title">
        <h3>Clients
        <a href="./create-customer.php" class="btn btn-sm btn-outline-primary float-end"><i class="fas fa-plus"></i> Ajouter</a>
        </h3>
    </div>
    <?php if($edit): ?>
        <div class="card">
            ...
            <div class="card-body">
                <form accept-charset="utf-8" method="post" action="<?= $_SERVER['REQUEST_URI'] ?>">
                    ...
                    <div class="row g-2">
                        ...
                        <div class="mb-3 col-md-4">
                            <input type="tel" name="telephone" class="form-control" placeholder="Teletelephone" value="<?= $customers[0]['telephone'] ?>">
                        </div>
                    </div>
                    <div class="mb-3">
