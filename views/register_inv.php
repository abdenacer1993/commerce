...
if (isset($_POST['register']) && CSRF::validateToken($_POST['token'])) {
  ...
  $createdTime = time();
  $role = 'investisseur';
  //----------------------------------- hadhom ihez fihom min formulaire ach 3amert fih 
  $statement = $pdo->prepare("SELECT * FROM personnes WHERE email=?");// ijbli min baz les info ili aandhom mail kif kif 
  $statement->execute(array($email));
  if ($statement->rowCount() > 0) {
    $error = true; //kan lga mail ili tapinah mwjoud bch igoli  rahou mawjoud
  } else { //kan lga lmail moch mawjoud iwali yaamili compte investisseur jdid 
    ...
    session_start();// bch yamili kif dar o i3abihali bili mawjoud fil compte ili 3mltou 
    ...
    header('Location: ./home.php');//ba3ed la i3abi dar ihzni lil page home 
  }
...
<head>
  ...
  <!-- bootstrap.min css -->
  <!-- ay page ma nadinch feha lheader lazem nadi feha bootstrap sinon juste 
  nadi header taw bootstrap ya5dem 5atrou mawjoud fil header -->
